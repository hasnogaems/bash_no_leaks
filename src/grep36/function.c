...
int grep(Flags flag,FILE* fp, int eflags, char* line_, char** argv, int y, int count,   int file_amount, char** f_ptrns, int* f_count, int argc){
 //printf("hello?\n");
    int x=0; 
    ...
    if (flag.e != 1 && flag.v != 1 && flag.c !=1 && flag.f !=1)
      while (fgets(line_, 1024, fp)) {
        ...
        flag.file_counter --;//уменьшить чтобы не печатало если паттерн безфлаговый с
        if (!x){
        if(flag.l==1){
        printf("%s\n", argv[optind]);
        break;}
